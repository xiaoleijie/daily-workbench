...
console.log('--- 回归 ---');
  ...
  T('R25-1: 单条修改弹窗 开始/结束 改原生 time 输入(es-st/es-en)',()=> html.indexOf('id="es-st" type="time"')>=0 && html.indexOf('id="es-en" type="time"')>=0 && html.indexOf('id="es-st-h"')<0 && html.indexOf('id="es-en-h"')<0);
  T('R25-2: 批量修改弹窗 开始/结束 改原生 time 输入(be-st/be-en)',()=> html.indexOf('id="be-st" type="time"')>=0 && html.indexOf('id="be-en" type="time"')>=0 && html.indexOf('id="be-st-h"')<0 && html.indexOf('id="be-en-h"')<0);
  T('R25-3: saveEditSchedItem 读原生时间(es-st/es-en)',()=> html.indexOf("var st=val('es-st')")>=0 && html.indexOf("timeVal('es-st-h'")<0);
  T('R25-4: saveBatchEdit 读原生时间(be-st/be-en)',()=> html.indexOf("var st=val('be-st')")>=0 && html.indexOf("timeVal('be-st-h'")<0);
  T('R25-5: hm24 归一化助手存在',()=> html.indexOf('function hm24(t)')>=0);
