...
setTimeout(()=>{
  ...
  T('① CSS: 明细日期字号 11px（独立 Georgia div）',()=>/font-family:Georgia,serif;color:var\(--muted\);font-size:11px;flex:0 0 auto/.test(html));
  ...
  T('② 拖拽只在 .sched-filter-cell 之间，不含「清除状态」',()=>{ const row=d.getElementById('schedFilters'); const cells=[].map.call(row.querySelectorAll('.sched-filter-cell'),c=>c.getAttribute('data-fk')); return cells.join(',')==='cat,status,date'; });
